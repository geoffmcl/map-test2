...
sub process_in_file($) {
    ...
    my $len = length($json_text);
    prt("Processing $lncnt lines, $len characters, from [$inf]...\n");
    if (VERB9()) {
        prt("[v9] Output of json string...\n");
        prt("$json_text\n");
    }
    my $perl_scalar = decode_json( $json_text );
    if (VERB5()) {
        prt("[v5] Dump of the perl scalar...\n");
